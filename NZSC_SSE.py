    ...
    def train(rng):
        ...
        for i in range(population_size - 1):
            ...
            agent_weight_path = os.path.join(
                f"ts_{config['NUM_STEPS']}_battleShip_view_{config['partner_agent_view_sizes'][i]}_v4_sigma_0",
        ...
        agent_weight_path = os.path.join(
            f"ts_{config['NUM_STEPS']}_battleShip_view_{config['ENV_KWARGS']['agent_1_view_size']}_v4_sigma_0",
...
def main(config):
    ...
    config['TRAINED_SEEDS'] = [0, 1, 2, 3, 4, 5, 6, 7] # Agent Seeds Trained via SelfPlay
    config['partner_agent_view_sizes'] = [2, 2, 2, 2, 2, 2, 2, 2] # Agents View Size
    for _ in range(5):

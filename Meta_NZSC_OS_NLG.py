...
def main(config):
    ...
    config['ENV_KWARGS']['sigma_rand'] = True
    config['ENV_KWARGS']['num_agent_steps'] = 1 # 1 for OS-NLG, 16 for I-NLG
    counter = 0
    for _ in range(5):
        run_name = "MetaNZSC_OS_NLG" + str(config["SEED"] + counter) + "_sigma_" + str(config['ENV_KWARGS']['sigma1']) + "_steps_" + str(config['ENV_KWARGS']['num_agent_steps'])
        config['SEED'] += counter

...
class BattleShipState:
    ...
    agent_1_comm_val: int
    agent_2_comm_val: int
    agent_comm_coord: bool
    noise_1: float
    noise_2: float
...
class BattleShipCommEnv(MultiAgentEnv):
    def __init__(self, num_agents=2, n_actions=5, agents=None, obs_size=None,
            ...
            action_spaces=None, observation_space=None, agent_view_size=3, sigma=2,
            ...
            agent_1_min_view_size=1, agent_1_max_view_size=3, agent_2_min_view_size=1, agent_2_max_view_size=3, include_rand_agent_1_view_size=True,
            include_rand_agent_2_view_size=True,
            agent_1_y_view_size=11, agent_2_y_view_size=11, rand_agent_1_view_size=[1, 3, 5, 7], rand_agent_2_view_size=[1, 3, 5, 7]):
            """
            Initialize the GridEnv environment with the specified parameters
            """
            ...
            self.num_comms_vals = 8
            self.move_actions = n_actions
            self.n_actions = self.move_actions + self.num_comms_vals
            ...
            self.include_rand_agent_1_view_size = include_rand_agent_1_view_size
            self.include_rand_agent_2_view_size = include_rand_agent_2_view_size
            self.rand_agent_1_view_size = rand_agent_1_view_size
            self.rand_agent_2_view_size = rand_agent_2_view_size
            ...
            if obs_size is None:
                ...
                obs_1_dim = obs_dim
                obs_2_dim = obs_dim
                self.agent_1_view_size_start = jax.lax.select(self.include_rand_agent_1_view_size, self.agent_1_max_view_size, agent_1_view_size) 
                self.agent_2_view_size_start = jax.lax.select(self.include_rand_agent_2_view_size, self.agent_2_max_view_size, agent_2_view_size)
                self.agent_1_y_view_size = agent_1_y_view_size
                self.agent_2_y_view_size = agent_2_y_view_size
                self.agent_1_view_size_start += 1
                self.agent_2_view_size_start += 1
                self.agent_1_obs_grid_size = (1) * self.agent_1_y_view_size
                self.agent_2_obs_grid_size = (1) * self.agent_2_y_view_size
    ...
    def _gen_grid(self, key: chex.PRNGKey, reward_1, reward_2, default_reward_box_pos):
        ...
        for i in jnp.arange(self.width):
            random_indices = jax.random.choice(keys[i], self.height, shape=(self.num_boxes_per_col,), replace=False)
    ...
    def reset(self, key: chex.PRNGKey) -> Tuple[Dict, BattleShipState]:
        """
        ...
        obs1 = -1 * jnp.ones((1, self.obs_1_size))
        obs2 = -1 * jnp.ones((1, self.obs_2_size))
        key, view_key1, view_key2, agent1_comm_key, agent2_comm_key = jax.random.split(key, num=5)
        min_max_values_agent_1 = jnp.array(self.rand_agent_1_view_size)
        min_max_values_agent_2 = jnp.array(self.rand_agent_2_view_size)
        noise_1 = jax.random.choice(key, jnp.array([0, -2, 2, -1, 1]), p=jnp.array([0.5, 0.1, 0.1, 0.15, 0.15]))
        noise_2 = jax.random.choice(key, jnp.array([0, -2, 2, -1, 1]), p=jnp.array([0.5, 0.1, 0.1, 0.15, 0.15]))
        self.agent_1_view = jax.lax.select(
            self.include_rand_agent_1_view_size,
            jax.random.choice(view_key1, min_max_values_agent_1),
            self.agent_1_view_size_start
        )

        self.agent_2_view = jax.lax.select(
            self.include_rand_agent_2_view_size,
            jax.random.choice(view_key2, min_max_values_agent_2),
            self.agent_2_view_size_start
        )
        ...
        default_reward_box_pos = jnp.tile(jnp.array([0]), (self.width, 1))
        ...
        agent_1_comm_val = jax.random.randint(agent1_comm_key, shape=(), minval=4, maxval=13)
        agent_2_comm_val = jax.random.randint(agent2_comm_key, shape=(), minval=4, maxval=13)
        ...
        obs1 = jax.lax.select(agent_b_pos_a_pov[0][0] != -1, obs1.at[:, 1:2].set(agent_2_comm_val), obs1.at[:, 1:2].set(jnp.array(-1).reshape(1, 1)))
        obs2 = jax.lax.select(agent_a_pos_b_pov[0][0] != -1, obs2.at[:, 1:2].set(agent_1_comm_val), obs2.at[:, 1:2].set(jnp.array(-1).reshape(1, 1)))
        ...
        obs1 = obs1.at[:, 11:12].set(agent_1_view)
        obs1 = obs1.at[:, 12:13].set(agent_2_view + noise_2)

        obs2 = obs2.at[:, 11:12].set(agent_2_view)
        obs2 = obs2.at[:, 12:13].set(agent_1_view + noise_1)
        ...
        state = BattleShipState(payoffs=payoffs, terminal_cond=terminal_cond, agent_b_pos_a_pov=agent_b_pos_a_pov, agent_a_pos_b_pov=agent_a_pos_b_pov,
                        ...
                        agent_1_comm_val=agent_1_comm_val, agent_2_comm_val=agent_2_comm_val, agent_comm_coord=False, noise_1=noise_1, noise_2=noise_2)
        return self.get_obs(state), state
    
    @partial(jax.jit, static_argnums=[0])
    def get_pos_moves(self, state: BattleShipState) -> chex.Array:
        """
        ...
        def _legal_moves(aidx: int, state: BattleShipState) -> chex.Array:
            ...
            actions = jnp.arange(self.move_actions)
            def grid_moves(i):
            ...
            possible_actions = jnp.ones(self.n_actions)
            possible_actions = possible_actions.at[0:5].set(moves)
    ...
    def cross_play_grid_view(self, agent_a_pos, agent_b_pos, grid_1, grid_2, agent_1_view, agent_2_view) -> chex.Array:
        ...
        def get_agent_reward_grid(agent_pos, view_size, agent_grid, grid):
            ...
            def body_func(loop_carry):
                ...
                agent_grid = agent_grid.at[i // (6), i % (6)].set(updated_grid_val)
        ...
        def get_agent_pos_grid(agent_pos, view_size, other_agent_pos):
            ...
            return agent_grid
        agent_a_grid = jnp.zeros((1, self.agent_1_y_view_size))
        agent_a_grid = get_agent_reward_grid(agent_a_pos, 1, agent_a_grid, grid_1)
        agent_b_grid = jnp.zeros((1, self.agent_2_y_view_size))
        agent_b_grid = get_agent_reward_grid(agent_b_pos, 1, agent_b_grid, grid_2)
    ...
    @partial(jax.jit, static_argnums=[0])
    def get_obs(self, state: BattleShipState) -> Dict:
    ...
    @partial(jax.jit, static_argnums=[0])
    def step_env(self, key: chex.PRNGKey, state: BattleShipState, actions: Dict,
                 ) -> Tuple[chex.Array, BattleShipState, Dict, Dict, Dict]:
        ...
        info_agent_0 = {
            ...
            "agent_comm_coord": state.agent_comm_coord,
        }
    ...
    def _update_pos(self, state: BattleShipState, actions: chex.Array) -> chex.Array:
        """Get updated players position based on their action"""
        @partial(jax.vmap, in_axes=[0, None])
        def _pos(agent_id: int, state: BattleShipState) -> Tuple[int, int]:
    ...
    def _get_other_agent_pos(self, curr_agent_pos: Tuple[int, int], other_agent_pos: Tuple[int, int], agent_num: int, agent_1_view, agent_2_view) -> chex.Array:
        ...
        agent_1_in_view = (-(curr_agent_pos[0] - agent_1_view + 1) > other_agent_pos[0])
        agent_2_in_view = (-(curr_agent_pos[0] - agent_2_view + 1) > other_agent_pos[0])
    ...
    def step_agent(self, key: chex.PRNGKey, state: BattleShipState, aidx: int, actions: chex.Array, coor: bool
                ) -> Tuple[BattleShipState, int]:
        ...
        agent_actions1, agent_actions2 = actions[0][0], actions[1][0]

        actions1 = jax.lax.select(agent_actions1>4, 4, agent_actions1)
        actions2 = jax.lax.select(agent_actions2>4, 4, agent_actions2)
        ...
        agent_comm_coord = (agent_actions1 > 3) & (agent_actions2 > 3) & (agent_actions1 == state.agent_2_comm_val) & (agent_actions2 == state.agent_1_comm_val)

        reward = jax.lax.select((idx_a!=-1) & (idx_a==idx_b) & agent_comm_coord, jnp.zeros((1, 1)).at[0].set(reward_values[idx_a]), state.non_prize_reward)
        ...
        obs1 = jax.lax.select(self.override_obs_with_zeros, -1 * jnp.ones((1, self.obs_1_size)), state.obs_vec_1)
        obs2 = jax.lax.select(self.override_obs_with_zeros, -1 * jnp.ones((1, self.obs_2_size)), state.obs_vec_2)
        ...
        key, agent1_comm_key, agent2_comm_key = jax.random.split(key, num=3)
        ...
        obs1 = obs1.at[:, 11:12].set(state.agent_1_view)
        obs1 = obs1.at[:, 12:13].set(state.agent_2_view+state.noise_2)

        obs2 = obs2.at[:, 11:12].set(state.agent_2_view)
        obs2 = obs2.at[:, 12:13].set(state.agent_1_view+state.noise_1)
        ...
        return state.replace(terminal_cond=terminal_cond, agent_b_pos_a_pov=agent_b_pos_a_pov, agent_a_pos_b_pov=agent_a_pos_b_pov, agent_steps=steps, 
                        ...
                        agent_0_last_non_stationary_move=agent_0_last_non_stationary_move, agent_1_last_non_stationary_move=agent_1_last_non_stationary_move, logging_reward=logging_reward,
                        agent_1_comm_val=agent_1_comm_val, agent_2_comm_val=agent_2_comm_val, agent_comm_coord=agent_comm_coord), reward

    def terminal(self, state: BattleShipState) -> bool:

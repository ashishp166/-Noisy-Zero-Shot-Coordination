...
class GridEnv(MultiAgentEnv):
    def __init__(self, num_agents=2, n_actions=5, agents=None, obs_size=None,
            ...
            max_rand_start_agent_x_pos=3, max_rand_start_agent_y_pos=3, include_time_step=False, agent_1_view_size=3, agent_2_view_size=3, min_rand_start_agent_x_pos=0, min_rand_start_agent_y_pos=0):
            """
    ...
    @partial(jax.jit, static_argnums=[0])
    def step_env_cross_play(self, key: chex.PRNGKey, state: GridState, actions: Dict,
    ...
    def step_agent(self, key: chex.PRNGKey, state: GridState, aidx: int, actions: chex.Array, coor: bool
        ...
        agent_1_box = jnp.float32(jax.lax.select(idx_a==idx_b, state.non_prize_reward, state.non_coordinating_reward))
        agent_2_box = jnp.float32(jax.lax.select(idx_b==idx_a, state.non_prize_reward, state.non_coordinating_reward))

        terminal_cond = jax.lax.select(state.agent_steps + 1 >= self.num_agent_steps, True, False)

        reward = jax.lax.select((idx_a!=-1) & (idx_a==idx_b), jnp.zeros((1, 1)).at[0].set(state.reward_map[2, idx_a]), agent_1_box + agent_2_box)
        ...
        return state.replace(terminal_cond=terminal_cond, agent_b_pos_a_pov=agent_b_pos_a_pov, agent_a_pos_b_pov=agent_a_pos_b_pov, agent_steps=steps, 
                        ...
                        agent_2_mid_reward_seen=agent_2_mid_reward_seen, agent_1_high_reward_seen=agent_1_high_reward_seen, agent_2_high_reward_seen=agent_2_high_reward_seen, logging_reward=logging_reward), reward

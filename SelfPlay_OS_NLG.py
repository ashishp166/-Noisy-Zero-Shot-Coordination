...
def main(config):
    ...
    config['ENV_KWARGS']['disable_other_play'] = True
    config['ENV_KWARGS']['num_agent_steps'] = 1 # 1 for OS-NLG, 16 for I-NLG
    counter = 0
    for _ in range(5):
        run_name = "SelfPlay_OS_NLG_" + str(config["SEED"] + counter) + "_sigma_" + str(config['ENV_KWARGS']['sigma1']) + "_steps_" + str(config['ENV_KWARGS']['num_agent_steps'])
        config['SEED'] += counter
        ...
        agent_weight = f"OS_NLG/agent_" + str(config["SEED"] + counter) + "_param_weights.npz"
